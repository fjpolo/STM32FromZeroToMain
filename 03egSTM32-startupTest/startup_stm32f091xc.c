...
void Reset_Handler() {
    ...
    uint32_t *dataInit = &_sidata;  // Alligned 4 byte 
...
__attribute__((section(".isr_vector")))             // Specific section
const void (*VectorTable[])(void) = {               // const to allocate in flash
    (const void (*)(void)) &_estack,                // typecast _estack 
    Reset_Handler,
    NMI_Handler,
    HardFault_Handler,
    0,
